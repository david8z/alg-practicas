...
def sparse_knapsack(W, v, w):
    ...
    def merge(list1, list2):
        """
        FunciÃ³n auxiliar, asume que list1 y list2 son listas de tuplas (peso,benef)
    ...
    for vi,wi in zip(v,w):
        # cada iteraciÃ³n es una etapa o columna de PD
...
def generate_instances(n_ins, W, n_obj):
    """
    Genera n_ins instancias del problema de la mochila discreta para un peso mÃ¡ximo
    de W y n_obj objetos
    Devuelve una lista con tantas tuplas como instancias: [(W, v, w), ...]
    """
    return [generateKnapsack(n_obj, W) for _ in range(n_ins)]
...
def measure_time(function, arguments,
    ...
    ordenar algo y lo deja ordenado, la prÃ³xima vez que ordenes no
    estarÃ¡ desordenado)

    DEVUELVE: tiempo y el valor devuelto por la funciÃ³n
    """
    count, accum = 0, 0
    while accum < 0.1:
...
def measure_knapsack_functions():
    ...
    Reportar, por salida estÃ¡ndar, para cada talla:

     - El tiempo medio de ejecución de cada algoritmo.
     - El porcenaje de estados activos visitados respecto de la versiÃ³n PD hacia atrÃ¡s.
    ...
    for n_obj in range(50,1001,50):
        ...
        for function,functname in functions:
            ...
            for i,instance in enumerate(instances):
                ...
                # muestra para cada ejecuciÃ³n una serie de valores:
    ...
    #
    # - comenta el print anterior y aÃ±ade cÃ³digo para mostrar los datos que se piden
